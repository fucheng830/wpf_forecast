...
def back_test(data, start_index=50000, train_len=3000, p=4, steps=16):
    ...
    pre_data = []
    rw = AR(data[start_index-train_len:start_index]).fit(p)
    for x in xrange(start_index,data.shape[0]-steps):
        ...
        #print ar_pre
        keep_pre = keep_predict(data, x, train_len, steps)
        pre_data.append([data.index[x+steps-1], ar_pre[steps-1], keep_pre[steps-1], data[x+steps-1]])

    result = pd.DataFrame(pre_data,columns=['ptime','ar','keep','p'])
...
def keep_predict(ser, x, train_len, steps):
    predict_ser = [ser[x-1] for i in range(steps)]
    return predict_ser
...
if __name__ == '__main__':
    ...
    df = read_history_data('data/%s_sbsj.xls'%dir_name)
    #df = pd.read_excel('data/%s_train.xlsx'%dir_name)
    df = df.dropna()
    #df.index = range(df.shape[0])
    df = df.set_index(keys='ptime',drop=False)
    df['p1'] = one_zero_normal(df['p'], 0, cap)
    #stationarity_test(df[100:200], cap)
    #print df[df['ptime']=='2017-01-01']
    #print df.iloc[2976]
    ...
    for i in range(19):
        ...
        data = []
        result_df = back_test(df['p1'],start_index=2976, train_len=2976, p=10, steps=i+1)
        result_df['p'] = df['p']
        ...
        for ptime, grouped in result_df.groupby(lambda x:'%s-%s-%s'%(x.year, x.month, x.day)):
            ...
            data.append([ptime, rmse(grouped['ar'], grouped['p'], cap), rmse(grouped['keep'],grouped['p'], cap)])
        new_df = pd.DataFrame(data,columns=[u'ptime',u'ar_rmse',u'keep_rmse'])
        ...
        step_result.append([i+1,new_df['ar_rmse'].mean(),new_df['keep_rmse'].mean()])#new_df['precision'].mean(),new_df['rmse_1'].mean()])
    ...
    df_avg = pd.DataFrame(step_result, columns=[u'step',u'ar_rmse',u'keep_rmse'])
    df_avg.to_excel('result/%s/%s_result.xlsx'%(dir_name,dir_name))
